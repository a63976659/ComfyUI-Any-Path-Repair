...
### 8. ⬇️ **可视化后台下载系统** (Visualized Background Download)

告别“盲盒”式下载，全新的下载引擎提供全方位的状态反馈：

* **按钮即进度条**：点击下载后，UI 按钮会实时变身进度条（如 `⏳ 45%`），背景色随进度动态增长。
* **控制台详情**：ComfyUI 控制台实时显示**下载速度、已下载大小**，让你对大模型下载心中有数。
* **随时中断**：下错文件或网速太慢？按钮变身红色“❌ 中断下载”，一键停止并自动清理残留文件。
* **后台持久化**：**关闭弹窗不影响下载！** UI 顶部醒目提示，你可以安心关闭页面继续作图，后台下载依然稳定运行。

### 9. 🎯 **URL 路径嗅探与物理锁定** (Smart Path Sniffing & Physical Locking)

这是解决“模型下错文件夹”的终极杀手锏。

* **智能嗅探**：插件不再盲目依赖 ComfyUI 的默认分类。它会深度分析下载链接，如果链接包含 `/diffusion_models/`、`/text_encoders/` 等关键词，它会**无视一切干扰**。
* **物理锁定**：直接绕过 ComfyUI 可能存在的注册表误导，强制将文件锁定到硬盘上的物理文件夹中。
* **解决痛点**：彻底修复了 `Acestep` 等新架构模型被错误下载到 `unet` 文件夹的问题。

### 10. 🏗️ **零依赖与纯净架构** (Zero Dependency & Clean Architecture)

为了极致的稳定性和易用性，我们重构了底层架构：

* **移除重型依赖**：完全移除 `huggingface_hub` 库，改用 Python 原生 `urllib`。用户**无需安装任何额外 Python 库**即可使用。
* **扁平化存储**：彻底解决了旧版库自动创建 `split_files` 等多级子文件夹的烦人问题，下载即用，无需手动移动文件。
* **内置镜像加速**：代码内置强制逻辑，自动将 `huggingface.co` 替换为 `hf-mirror.com`，**国内用户无需配置代理**即可享受满速下载。

### 11. 🖱️ **人性化交互细节** (Enhanced UX Details)

针对重度用户的贴心优化：

* **一键复制直链**：下载按钮旁新增 `📋 复制链接`，方便习惯使用 **迅雷/IDM** 的用户手动下载。
* **防溢出滚动设计**：当缺失模型过多时，弹窗自动启用垂直滚动条并限制最大高度，防止撑爆屏幕。
* **空文件自动熔断**：下载完成后自动校验，如果发现是 0KB 的空文件（如网络错误导致），自动报错并清理，防止损坏的模型污染文件夹。
...
3. 新增对 LoadMoGeModel（MoGe几何估计模型）加载器的支持，修复MoGe模型路径问题。
...
### **2026-02-01** `v1.3.0`
1. 智能路径精准锁定：自动下载到对应的文件夹，无需手动选择。
2. 可视化后台下载系统：实时下载状态反馈，可多个模型同时下载。
随时中断，关闭了插件弹窗，后台下载依然，再次点击修复按钮可查看状态，直至下载完成。
3. 增强型交互界面：防溢出滚动设计，顶部醒目提示，消除用户不懂如果使用的顾虑。
4. 架构重构与零依赖：不需要安装额外 Python 库就能用，
内置将 huggingface替换为 hf-mirror，国内外用户都可满速下载。
